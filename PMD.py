...
class Database:
    def __init__(self):
        ...
        self.dbCursor.execute("CREATE TABLE IF NOT EXISTS PUBLIC_INFO (id PRIMARYKEY text, fName text, lName text, dob text, mob text, yob text, gender text, address text, phone text, email text, bloodGroup text, history text, doctor text)")
    ...
    def Insert(self, id, fName, lName, dob, mob, yob, gender, address, phone, email, bloodGroup, history, doctor):
        self.dbCursor.execute("INSERT INTO PUBLIC_INFO VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (id, fName, lName, dob, mob, yob, gender, address, phone, email, bloodGroup, history, doctor))
        self.dbConnection.commit()
        
    def Update(self, fName, lName, dob, mob, yob, gender, address, phone, email, bloodGroup, history, doctor, id):
        self.dbCursor.execute("UPDATE PUBLIC_INFO SET fName = ?, lName = ?, dob = ?, mob = ?, yob = ?, gender = ?, address = ?, phone = ?, email = ?, bloodGroup = ?, history = ?, doctor = ? WHERE id = ?", (fName, lName, dob, mob, yob, gender, address, phone, email, bloodGroup, history, doctor, id))
        self.dbConnection.commit()
        
    def Search(self, id):
        self.dbCursor.execute("SELECT * FROM PUBLIC_INFO WHERE id = ?", (id, ))
        searchResults = self.dbCursor.fetchall()
        return searchResults
        
    def Delete(self, id):
        self.dbCursor.execute("DELETE FROM PUBLIC_INFO WHERE id = ?", (id, ))
        self.dbConnection.commit()

    def Display(self):
        self.dbCursor.execute("SELECT * FROM PUBLIC_INFO")
        records = self.dbCursor.fetchall()
        return records

class Values:
    def Validate(self, id, fName, lName, phone, email, history, doctor):
        if not (id.isdigit() and (len(id) <= 15)):
            return "id"
...
class InsertWindow:
    def __init__(self):
        ...
        # Labels
        tkinter.Label(self.window, text = "ADHAAR ID",  width = 25).grid(pady = 5, column = 1, row = 1)
        ...
        tkinter.Label(self.window, text = "MEDICAL History",  width = 25).grid(pady = 5, column = 1, row = 12)
...
class UpdateWindow:
    def __init__(self, id):
        ...
        # Labels
        tkinter.Label(self.window, text = "PUBLIC ID",  width = 25).grid(pady = 5, column = 1, row = 1)
        ...
        tkinter.Label(self.window, text = "Email ID",  width = 25).grid(pady = 5, column = 1, row = 10)
        tkinter.Label(self.window, text = "Blood Group",  width = 25).grid(pady = 5, column = 1, row = 11)
        tkinter.Label(self.window, text = "PUBLIC History",  width = 25).grid(pady = 5, column = 1, row = 12)
        tkinter.Label(self.window, text = "Doctor",  width = 25).grid(pady = 5, column = 1, row = 13)
...
class DatabaseView:
    def __init__(self, data):
        ...
        self.databaseView.column("address", width = 150)
        self.databaseView.column("phone", width = 100)
        self.databaseView.column("email", width = 150)
        self.databaseView.column("bloodGroup", width = 100)
        self.databaseView.column("history", width = 150)
...
class SearchDeleteWindow:
    def __init__(self, task):
        ...
        self.fName = tkinter.StringVar()
        self.lName = tkinter.StringVar()
        self.heading = "Please enter PUBLIC ID to " + task

        # Labels
        tkinter.Label(window, text = self.heading, width = 50).grid(pady = 20, row = 1)
        tkinter.Label(window, text = "PUBLIC ID", width = 10).grid(pady = 5, row = 2)
...
class HomePage:
    def __init__(self):
        ...
        self.homePageWindow.wm_title("PUBLIC MEDICAL DATA")
